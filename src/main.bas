...
675 IF DEBUG > 3 THEN HTAB(76) : VTAB(1) : PRINT XX;"*";YY;
682 IF DEBUG > 3 THEN GOSUB 23000 : PRINT : REM LIMIT SCROLL AREA
685 ZZ = 0
...
770 INSIDES(X, Y) = 1 : VISIBLES(X,Y) = 1 : MAP(X,Y) = 1 : X1 = 5 + 9*(X-1)

775 REM MARK THE LEVEL ENTRANCE AND THE ROOM ON THE RIGHT AS VISITED AS WELL (HARDCODING EXAMPLE)
780 REM INSIDES(1 + INT(SIZE / 2), SIZE) = 1 : VISIBLES(1 + INT(SIZE / 2), SIZE) = 1 : MAP(1 + INT(SIZE / 2), SIZE) = 4
785 REM INSIDES(2 + INT(SIZE / 2), SIZE) = 3 : VISIBLES(2 + INT(SIZE / 2), SIZE) = 1 : MAP(2 + INT(SIZE / 2), SIZE) = 3
790 REM STACK = STACK + 2
795 REM SX(1) = 1 + INT(SIZE / 2) : SY(1) = SIZE
800 REM SX(2) = 2 + INT(SIZE / 2) : SY(2) = SIZE
...
1199 REM IF NO UNVISITED NEIGHBRS - BACKTRACK
1200 IF TYPE = 1 THEN GOTO 1220
1205 SX(STACK) = 0 : SY(STACK) = 0 : STACK = STACK - 1 : INDEX = STACK
1210 IF DEBUG > 3 THEN GOSUB 23000 : VTAB(20) : HTAB(2) : PRINT "Removed ";STACK;" at ";X;"x";Y; ", get ";SX(STACK);"x";SY(STACK) : GOSUB 23100
1215 GOTO 1230

1220 SX(INDEX) = 0 : SY(INDEX) = 0 : STACK = STACK - 1
1225 IF DEBUG > 3 THEN GOSUB 23000 : VTAB(20) : HTAB(2) : PRINT "Removed ";INDEX;" at ";X;"x";Y; ", get ";SX(INDEX);"x";SY(INDEX) : GOSUB 23100
...
1500 HTAB(OFFSET) : VTAB(22) : PRINT "_";
...
2070 XX = 0 : YY = 0 : ZZ = 0 : REM VARIABLES TO BE USED IN SUBROUTINES LIKE THE ALTERNATIVE HTAB OR WHILE GENERATING A STAGE
...
2235 IF DEBUG > 0 THEN GOSUB 280 : REM GC
2240 REM IF DEBUG > 0 THEN PRINT D$"BLOAD READCHAR" : REM LOAD HERE IF SKIPPED

2245 REM GOSUB 22000 : REM GO GENERATE A STAGE BEFORE LOADING THE TITLE PICTURE

2247 REM FF$ = "SCREEN" : GOSUB 26000
...
6035 KY = PEEK(49152) : REM GET LAST PRESSED KEY AND DISPLAY DEBUG CURSOR INFO
...
6080 POKE 2039, 64 : REM PRINT AN APPLE ICON AT BOTTOM RIGHT CORNER OF SCREEN (POKING TO PREVENT SCROLL)

6300 GOSUB 23000 : REM LIMIT SCROLL AREA

6310 CALL 768 : IF DEBUG > 4 OR DEBUG = 2 THEN RNG = RND(-1*(PEEK(78)+256*PEEK(79)))

6320 HOME
6350 FOR I = 1 TO 6 : PRINT LINES$(5, I) : NEXT I
6360 PRINT : FOR I = 7 TO 11 : PRINT LINES$(5, I) : NEXT I
6380 GOSUB 23100

6400 W = 3 : XX = 8 : GOSUB 25000 : REM DRAW BEGIN JOURNEY WINDOW WITH RANDOM SEED PLACEHOLDER
6410 GOSUB 280 : REM DISPLAY FREE MEMORY
6420 HTAB(67) : VTAB (18) : PRINT MI$;PEEK(78);NI$ : HTAB(74) : VTAB (18) : ? MI$;PEEK(79);NI$ : REM DISPLAY SEED VARS
...
6699 REM SHUFFLE THE LETTERS ARRAY USING RANDOM SWAPS
6700 FOR I = 1 TO 15
...
6750 REM NEXT I
6760 PRINT CHR$(63 + N2%(1)); : REM PRINT ONLY 1 CHARACTER AFTER THE SHUFFLED NUMBERS

6765 REM HTAB(22) : VTAB (23) : ? "Randomize:";M$;"Z";N$;MI$;" > Press Any Key < ";NI$;M$;"_";N$;"- Requirement";
6770 XX = 8 : GOSUB 1500 : REM CLEAR PROGRESS BAR
6780 GOSUB 22000 : REM GENERATE STAGE
6790 XX = 1 : GOSUB 200 : W = 3 : XX = -8 : GOSUB 25000 : REM DRAW BEGIN JOURNEY WINDOW AGAIN WITH THE BOTTOM MENU UI

6800 CALL 768 : REM RNG = RND(-1*(PEEK(78)+256*PEEK(79))) : REM RE-SEED THE RND FUNCTION
6850 KY = PEEK(49152) : REM GET LAST PRESSED KEY AND DISPLAY DEBUG CURSOR INFO
6870 IF KY = 32 THEN GOTO 6400
6880 IF KY = 13 THEN GOTO 6900
6890 GOTO 6850

6899 REM INITIALIZE THE GAME WITH THE RANDOM SEED
6900 HTAB(1) : VTAB(21) : CALL -958 : REM CLEAR TEXT FROM CURSOR TO BOTTOM OF WINDOW
6920 KY = PEEK(49152) : REM GET LAST PRESSED KEY AND DISPLAY DEBUG CURSOR INFO
6950 REM W = 1 : GOSUB 25000 : REM DRAW PLACEHOLDER
6955 GOSUB 280 : REM FRE
...
7030 X = X1 : Y = 19 : REM INITIAL PLAYER POSITION XX
...
9970 W = 4 : XX = Z : GOSUB 25000 : REM RENDER GAME WELCOME SCREEN
9975 IF DEBUG = 1 THEN DEBUG = 3 : GOSUB 30500 : DEBUG = 1 : REM RENDER DEBUG STAGE DISPLAY : TEMP
9978 IF DEBUG = 2 THEN DEBUG = 3 : GOSUB 30500 : DEBUG = 2
9980 IF DEBUG > 2 THEN GOSUB 30500
...
9997 REM MAIN LOOP
9998 REM FOR II=1 TO 200: NEXT II : REM WAIT 49200,3
9999 REM IF PLAYER(1) = X AND PLAYER(2) = Y THEN GOTO 550 : REM SKIP IF PLAYER HAS NOT MOVED
...
11800 IF K = 21 OR K = 149 OR K = 68 OR K = 196 OR K = 100 OR K = 228 OR K = 76 OR K = 204 OR K = 108 OR K = 236 THEN GOTO 12500 : REM MOVING = 1 : MDIR = 1 : GOTO 12500 : REM RIGHT
11900 IF K = 8 OR K = 136 OR K = 65 OR K = 193 OR K = 97 OR K = 225 OR K = 74 OR K = 202 OR K = 106 OR K = 234 THEN GOTO 13000 : REM MOVING = 1 : MDIR = -1 : GOTO 13000 : REM LEFT
12050 IF K = 11 OR K = 139 OR K = 87 OR K = 215 OR K = 119 OR K = 247 OR K = 73 OR K = 201 OR K = 105 OR K = 233 THEN GOTO 13500 : REM UP
12100 IF K = 10 OR K = 138 OR K = 83 OR K = 211 OR K = 115 OR K = 243 OR K = 75 OR K = 203 OR K = 107 OR K = 235 THEN GOTO 14000 : REM DOWN
...
21500 HTAB (80 - LEN( ST$ ))/2 : REM CENTERING ALGORITHM
21600 REM VTAB B
21700 PRINT ST$
21800 RETURN
...
22175 GOSUB 30500 : REM DEBUG DISPLAY THE STAGE INITIALLY ?
22180 FF = 0 : GOSUB 750 : REM GENERATE STAGE

22190 IF DEBUG > 3 THEN GOSUB 23000 : VTAB(20) : HTAB(2) : PRINT "Generation complete!" : GOSUB 23100
22198 RETURN : REM FINAL RETURN TO CALLER
...
23999 REM DEBUG DISPLAY AT THE BOTTOM
24000 IF DEBUG = 0 OR X = 0 THEN RETURN
24010 HTAB(3) : VTAB(24) : ? MI$;"  cursor: ";X;"x";Y;NI$;M$;"N";N$;MI$;" PLR symbol: ";A;
...
25000 YY = 1 : IF XX < 0 THEN YY = XX * -1 : XX = Z
25100 FOR I = YY TO XX
25550    REM HTAB(42) : VTAB(24) : PRINT "xy:";COODS(W, 2);" x ";(COODS(W, 3) + I);
25600    HTAB(COODS(W, 2)) : VTAB (COODS(W, 3) + I)
25700    IF I < XX THEN PRINT LINES$(W, I)
25750    IF I = XX THEN PRINT LINES$(W, I);
25800 NEXT I
25900 RETURN
...
30545       HTAB(49 + XX) : VTAB(15 + YY) : O = 1
...
30595       HTAB(55 + XX) : VTAB(15 + YY) : IF XX = 1 THEN PRINT CHR$(8);M$;"N";N$;
30600       IF X = XX AND Y = YY THEN PRINT MI$;VISIBLES(XX, YY);NI$; : NORMAL : GOTO 30610
30605       PRINT VISIBLES(XX, YY);M$"N";N$;
30610    NEXT XX
30615 NEXT YY
