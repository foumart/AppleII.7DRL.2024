...
3350 HTAB(62) : INPUT A$ : REM GET THE NEXT DATA COL
...
9000 REM GOSUB 20000 : REM RENDER GAME MAP

9499 REM CLEAR GAME SCREEN
 
9970 W = 4 : X1 = Z : GOSUB 25000 : REM RENDER GAME WELCOME SCREEN
9980 POKE 2039, 76 : REM PRINT AN UPPER BORDER TO FINALIZE UI AT BOTTOM RIGHT CORNER OF SCREEN (POKING TO PREVENT SCROLL)
...
15999 REM CLEAR A ROOM
16000 REM WW = MX+1 : GOSUB 200
16010 REM VTAB(MY+1)
16020 REM M = MAP(RX+1, RY+1)
16050 REM V = VISIBLES(RX+1, RY+1)

17400 REM XX = MX+1 : GOSUB 200 : VTAB(MY+1) : ? "         "
17410 REM HTAB(MX+1) : VTAB(MY+2) : ? "         "
17430 REM HTAB(MX+1) : VTAB(MY+3) : ? "         "
17450 REM HTAB(MX+1) : VTAB(MY+4) : ? "         "
17460 REM HTAB(MX+1) : VTAB(MY+5) : ? "         "
...
19999 REM RENDER ALL MAP ROOMS ROUTINE (DISCONTINUED IN THIS FILE - CURRENTLY IN GAME)
20000 REM FOR RY = 0 TO 4
20100   REM FOR RX = 0 TO 4
20200     REM CALCULATE THE X AND Y COORDINATES OF THE TOP LEFT CORNER OF THE ROOM
20300     REM MX = (RX) * (RW)
20400     REM MY = (RY) * (RH)
20500     REM CLEAR A ROOM AT THE CURRENT POSITION
20600     REM GOSUB 16000
20700   REM NEXT RX
20800 REM NEXT RY
20900 REM RETURN
...
22199 REM RESET VISIBILITY (IT WAS USED DURING GENERATION LOOP, BUT NOW WE NEED IT FOR THE GAME)
...
22799 REM OPEN THE ROOM NEXT TO THE ENTRANCE ROOM
22800 REM IF X1 > 1 THEN GOTO 22810
22805 REM GOTO 22820
22810 REM IF (MAP(X1, Y1) = 3 OR MAP(X1, Y1) = 4 OR MAP(X1, Y1) = 8) THEN VISIBLES(X1 - 1, Y1) = 1

22820 REM IF X1 < SIZE-1 THEN GOTO 22830
22825 REM GOTO 22840
22830 REM IF (MAP(X1+1, Y1) = 3 OR MAP(X1+1, Y1) = 4 OR MAP(X1+1, Y1) = 8)  THEN VISIBLES(X1 + 1, Y1) = 1

22840 REM IF Y1 > 1 THEN GOTO 22850
22845 REM GOTO 22860
22850 REM IF (MAP(X1, Y1) = 2 OR MAP(X1, Y1) = 4 OR MAP(X1, Y1) = 9) THEN VISIBLES(X1, Y1 - 1) = 1

22860 REM IF Y1 < SIZE-1 THEN GOTO 22870
22865 REM GOTO 22890
22870 REM IF (MAP(X1, Y1+1) = 2 OR MAP(X1, Y1+1) = 4 OR MAP(X1, Y1+1) = 9) THEN VISIBLES(X1, Y1 + 1) = 1
...
25700    IF I < X1 THEN PRINT LINES$(W, I);CHR$(8)
25750    IF I = X1 THEN PRINT LINES$(W, I);CHR$(8);
25800 NEXT I
25900 RETURN
