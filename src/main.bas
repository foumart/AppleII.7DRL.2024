...
279 REM RUN GC AND DISPLAY FREE MEMORY LEFT IF IN DEBUG MODE
280 IF FF = 1 THEN AVAIL = FRE(0)
284 IF FF = 2 THEN ? D$;"FRE"
286 FF = 0
288 IF DEBUG = 0 THEN RETURN
290 IF DEBUG > 0 THEN HTAB(64) : VTAB (24) : PRINT MI$;"free:";INT(AVAIL/1000);"k ";AVAIL;NI$;" ";
295 RETURN
...
675 IF DEBUG > 2 THEN HTAB(76) : VTAB(1) : PRINT XX;"*";YY;
682 IF DEBUG > 2 THEN GOSUB 23000 : PRINT : REM LIMIT SCROLL AREA
685 ZZ = 0
...
740 SX(STACK) = XX
745 SY(STACK) = YY
746 IF DEBUG > 2 THEN GOSUB 23100 : REM RESTORE SCROLL AREA
748 RETURN

749 REM DEPTH-FIRST SEARCH ALGORITHM FOR GENERATING A RANDOMIZED INTERCONNECTED ROOM MAP
750 X = INT(RND(1) * SIZE) + 1: Y = 1 : REM INITIAL ROOM PICK (EXIT)
765 REM MARK THE LEVEL EXIT CELL AS VISITED
770 INSIDES(X, Y) = 2 : VISIBLES(X,Y) = 1 : MAP(X,Y) = 1
775 REM MARK THE LEVEL ENTRANCE AND THE ROOM ON THE RIGHT AS VISITED AS WELL
780 INSIDES(1 + INT(SIZE / 2), SIZE) = 3 : VISIBLES(1 + INT(SIZE / 2), SIZE) = 1 : MAP(1 + INT(SIZE / 2), SIZE) = 4
785 INSIDES(2 + INT(SIZE / 2), SIZE) = 1 : VISIBLES(2 + INT(SIZE / 2), SIZE) = 1 : MAP(2 + INT(SIZE / 2), SIZE) = 3
790 STACK = STACK + 2
...
1230 IF SX(STACK) > 0 THEN X = SX(STACK) : Y = SY(STACK) : GOTO 810
1240 RETURN
...
2245 GOSUB 22000 : REM GO GENERATE THE STAGE

2247 POKE 49237,0: PRINT D$;"BLOAD SCREEN, A$2000, L$2000"
2248 POKE 49236,0: PRINT D$;"BLOAD SCREEN, A$2000, L$2000, B$2000" 
...
6000 TEXT : REM SITCH TO TEXT MODE NOW
...
6075 HTAB(52) : VTAB (24) : CALL -868 : GOSUB 280 : REM CLEAR AND GC
...
6349 REM PRINT THE STORY LINES DIRECTLY
...
9975 IF DEBUG = 1 THEN DEBUG = 2 : GOSUB 30500 : DEBUG = 1 : REM RENDER DEBUG STAGE DISPLAY : TEMP
...
12050 IF K = 139 OR K = 247 OR K = 215 THEN GOTO 13500
...
21998 REM GENERATE MAP ROUTINE
21999 REM DEBUG 0: SILENT; 1: JUST DISPLAY LEVEL; 2: DISPLAY DEBUG INFO; 3: ALSO WAIT FOR USER INTERACTION
22000 IF DEBUG > 2 THEN VTAB(19) : HTAB(50) : PRINT "Hit Any Key..." : CALL 768 : RNG = RND(-1*(PEEK(78)+256*PEEK(79)))
22105 DIM AVAILABLE(4) : AVAILABLE(1) = 0 : AVAILABLE(2) = 0 : AVAILABLE(3) = 0 : AVAILABLE(4) = 0 : REM 4 DIRECTIONS
22110 SIZE = 5 : REM INITIALIZE MAZE DIMENSIONS
22115 DIM MAP(SIZE,SIZE) : REM GENERATE MAP CONNECTIONS ARRAY
22120 DIM VISIBLES(SIZE,SIZE) : REM GENERATE MAP VISITED ROOMS ARRAY
22122 DIM INSIDES(SIZE,SIZE) : REM GENERATE MAP CONTENTS ARRAY
22125 DIM SX(SIZE*SIZE)
22130 DIM SY(SIZE*SIZE)

22135 TYPE = 0 : REM 0: LEAD CELL GENERATION (ZIG-ZAG LONGER); 1: RANDOM CELL (MORE CHAOTIC) : TODO TO IMPLEMENT
...
22180 GOSUB 750 : REM GENERATE STAGE

22190 REM IF DEBUG = 0 THEN HOME : DEBUG = 1 : GOSUB 30500
22195 IF DEBUG > 2 THEN GOSUB 23000 : VTAB(20) : HTAB(2) : PRINT "Generation complete!" : GOSUB 23100
...
30500 IF DEBUG < 2 THEN GOTO 30670
...
30610    NEXT XX
30615 NEXT YY
30620 IF DEBUG < 3 THEN GOTO 30670
30622 IF DEBUG > 3 THEN GOTO 30668
